...
describe('StepLine', () => {
  ...
  it('offers the instruction, note, time and temperature', async () => {
    await render(<StepLine line={line} onChange={jest.fn()} />);
  ...
  it('reports a change with the rest of the line intact', async () => {
    ...
    await render(<StepLine line={line} onChange={onChange} />);
  ...
  it('shows a body error in words', async () => {
    await render(
      <StepLine line={{ ...line, body: '' }} errors={{ body: true }} onChange={jest.fn()} />,
    );
