...
- [ ] Share on a ready recipe answers a 12-character token and a `panna://` url, the same token
      again on a second call; on a draft, 409; another user's recipe, 404, asserted end to end.
- [ ] `GET /api/shared/:token` needs no session and returns the recipe without notes or cook
      counts; after revoke it is 404; a made-up token is 404, asserted end to end.
- [ ] Save copies the recipe to the caller as a draft with every ingredient, piece of equipment,
      ...
      leaves the copy whole with its photos, asserted end to end.
- [ ] The recipe screen shows Share for a ready recipe and nothing for a draft; Stop sharing asks
      and revokes, asserted in a test.
- [ ] The shared screen shows the recipe and "Add to my recipes", and lands on the copy; a
      revoked link shows the no-longer-works state, asserted in a test.
- [ ] On the simulator: share the soup as Jānis, open the link as Anna, add it, see it in Anna's
