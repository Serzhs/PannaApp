...
- The review page of the create flow and marking a recipe ready. See the open questions.
...
- [ ] `GET /api/recipes/:recipeId` returns `steps` as `[]` for a recipe with none, and `GET /api/recipes` carries no steps.
- [ ] A `PATCH` with three main steps, the second with two nested, stores main positions 0, 1, 2 and nested positions 0, 1 under the second, and `GET` returns the same nesting and order.
- [ ] A `PATCH` that reorders the main steps and the nested steps rewrites every position and keeps every id.
- [ ] A `PATCH` that lists a nested step as a main step, and one that lists a main step under another, each move the same id rather than creating a new row.
- [ ] A `PATCH` that omits a main step and lists its former children as main steps keeps those children with their ids: the delete promoted rather than cascaded.
- [ ] A `PATCH` that omits a main step and its children deletes all of them.
- [ ] A body with a nested step that itself carries `children` returns 400 and writes nothing.
- [ ] A `PATCH` whose third step's nested second step has a blank body returns 400 with `fields` naming `steps.2.children.1.body`, and writes nothing.
- [ ] `durationSeconds` of `0`, of `90000` and of `1.5`, and `temperatureCelsius` of `501`, each return 400.
- [ ] A `PATCH` with 61 main steps, and one with 21 nested steps under one main step, each return 400 and write nothing.
- [ ] `totalTimeMinutes` is the sum of the main steps' durations rounded up to a minute after a `PATCH`, is null when no main step is timed, ignores nested steps' durations, and updates when a step's duration changes.
- [ ] `POST /api/recipes` and `PATCH /api/recipes/:recipeId` with `totalTimeMinutes` in the body return 400.
- [ ] A `PATCH` carrying a step `id` from another user's recipe returns 400 and changes neither recipe; a `PATCH` on another user's recipe returns 404 with the same body as an unknown id.
- [ ] The recipe screen numbers main steps from 1, shows nested steps indented and labelled as meanwhile with 2a-style numbers, and shows 180 as 356°F to an imperial reader, asserted in a component test.
- [ ] The recipe screen reads each step as one element carrying its number, body, timing and note, asserted by querying the accessible name.
- [ ] The total time field is absent from the create and edit forms, asserted in a test, and the recipe screen's total is the derived one.
- [ ] In the app, adding a main step, nesting a step under it, promoting it back, moving steps within a level and removing a main step with nested steps all behave as the UI section says, and saving shows the result on the recipe screen without a manual refresh.
- [ ] After "What you need" in the create flow, the app continues to "Steps"; Done saves and lands on the recipe; Skip lands on the recipe with no steps.
- [ ] Every new component appears in the design gallery in every state.
- [ ] The Latvian file lists every new key, and switching language changes the section headings and the meanwhile label.
...
1. **Is this one spec or two?** The whole of this is two days: the contract, the transactional write with promotion, the derived total, the read view, an editor with two levels, a native duration picker, and a third create page. A clean split is 0008 for the contract, the API, the derived total and the read view, and a following spec for the editor and the create page; the read view would show steps that only the import in 0015 or the database could create until the editor lands. One spec means nothing ships until all of it does.
2. **What does the temperature field take?** Storing Celsius is settled. An author on imperial units could type Fahrenheit into a field labelled °F, converted to Celsius on save and back on read - 350 becomes 177 becomes 351, a degree of drift every round trip - or every author types Celsius and the field says so, which is simpler and honest but asks a US author to convert in their head.
3. **Where does "mark as ready" live?** `status` has had an API since 0005 and no control in the app. The create flow's fourth page, a review, was always the intended place; it could be a small addition to the create page here, a checkbox on the edit screen, or its own spec after this one.
