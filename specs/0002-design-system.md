...
- [x] Text scales with the OS font size setting, and at the largest setting nothing in the gallery is clipped or truncated mid-word. _(Verified on the iPhone 17 Pro simulator at the largest accessibility size, 23 September 2026. It found three places that did not survive: the list header, whose height iOS fixes; the avatar's letter in its fixed circle; and the recipe row's title beside its chips. The first two carry `FIXED_LAYOUT_MAX_FONT_SCALE`, the one sanctioned cap, named so every use can be found; the row lets its chips wrap under the title instead. The You tab's email now sits above Save rather than beside it.)_
